...
V5 将图片清晰度直接视为功能要求。桌面宽屏背景不再从旧的 864×540 中间 WebP 放大，而是重新从原始卧铺舱图片制作并以内嵌 3172×1984 高质量 WebP 交付。界面也不再依赖整屏磨砂蒙版，文字可读性由局部半透明控件和更清晰的侧边栏承担。

- 3172×1984 高清宽屏背景，直接来自原始卧铺舱图片。
- 使用高质量 WebP 压缩，避免低分辨率素材反复转码。
...
V5 将清晰度、局部对比和外壳可用性视为功能要求。宽屏背景必须保持至少 3000×1900；皮肤 CSS 不得模糊摄影层或应用外壳；composer seat 不得在真正输入框后重新绘制第二层横向背景。
